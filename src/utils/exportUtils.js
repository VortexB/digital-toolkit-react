...
export const collectRecommendedActions = async (answers, group) => {
  ...
  return collectedData.sort((a, b) => {
    if (a.subject !== b.subject) {
      return (subjectOrder[a.subject] ?? 999) - (subjectOrder[b.subject] ?? 999);
    }
...
const renderTextWithLinks = (doc, text, x, startY, maxWidth, lineHeight = 5, dryRun = false) => {
  const linkRegex = /\[([^\]]+)\]\(([^)]+)\)/g;
  ...
  for (const segment of segments) {
    if (segment.type === 'text') {
      ...
      if (currentLineWidth > 0 && currentLineWidth + spaceWidth + linkWidth > maxWidth) {
        flushLine();
      }
      ...
      currentLineWidth += (currentLineWidth > 0 ? spaceWidth : 0) + linkWidth;
    }
  }
  flushLine();
  ...
  for (const line of lines) {
    ...
    for (const segment of line) {
      if (segment.type === 'text') {
        ...
        doc.line(currentX, yPosition + 1, currentX + segment.width, yPosition + 1);
        
        // Add clickable link
        doc.link(currentX, yPosition - 3, segment.width, 6, { url: segment.url });
...
const addPageFooter = (doc, pageNum, totalPages, pageWidth) => {
  const footerY = 285;
...
// Draw cover page
const drawCoverPage = (doc, user, pageWidth, pageHeight, logoImages = {}) => {
  ...
  doc.text("D3SM Digital Implementation Toolkit", centerX, yPos, { align: "center" });
  yPos += 15;
  ...
  doc.setFont("Lato", "normal");
  doc.setTextColor(33, 37, 41);
  const titleLines = doc.splitTextToSize(user.projectTitle || "Not specified", pageWidth - 70);
  ...
  doc.setFont("Lato", "normal");
  doc.setTextColor(33, 37, 41);
  const locationParts = [user.projectCountry, user.projectProvince, user.projectCity].filter(Boolean);
  ...
  doc.setFontSize(9);
  doc.setTextColor(100, 100, 100);
  const description = "This D3SM toolkit report aims to provide ideas, steps, and resources to support researchers, clinicians, health care managers, and project leads when planning, implementing, and evaluating digital mental health innovations. It also considers how to plan sustainability through implementation. We created this toolkit by adapting the non-adoption, abandonment and challenges to scale-up, spread, sustainability (NASSS) framework, combined with a Complexity Assessment Tool (CAT), developed by Greenhalgh and her colleagues.";
  ...
  doc.setFontSize(10);
  doc.setTextColor(100, 100, 100);
  doc.text(`Report generated: ${new Date().toLocaleDateString('en-CA', { 
    year: 'numeric', 
    month: 'long', 
    day: 'numeric' 
...
export const generatePDF = async (collectedData, user) => {
  ...
  // Draw cover page
  drawCoverPage(doc, user, pageWidth, pageHeight, logoImages);
  ...
  doc.setFontSize(9);
  doc.setTextColor(100, 100, 100);
  doc.text("The following results are organized by domain. Each section shows your responses and recommended actions.", 20, yPosition);
  yPosition += 15;
  ...
  if (collectedData.length === 0) {
    ...
    collectedData.forEach((item, index) => {
      ...
      // Question text (with link support)
      doc.setFont("Lato", "normal");
      doc.setFontSize(9);
      doc.setTextColor(73, 80, 87);
      yPosition = renderTextWithLinks(doc, item.questionText, 25, yPosition, pageWidth - 50, 4.5);
      yPosition += 6;
      ...
      if (item.isGoodJob) {
        ...
        doc.text("No additional actions needed for this item. Good Job!", 25, yPosition);
        yPosition += 6;
        doc.setTextColor(0, 0, 0);
      } else if (item.needsActions && item.actions.trim()) {
        // Recommended actions header
        ...
        const tempY = renderTextWithLinks(doc, item.actions, 30 + actionsPadding, yPosition, pageWidth - 60 - actionsPadding * 2, 4, true);
        ...
        doc.roundedRect(25, actionsStartY, pageWidth - 50, actionsHeight, 2, 2, "FD");
        
        // Render actions text with interior margin (actual rendering)
        doc.setFont("Lato", "normal");
        doc.setFontSize(9);
        doc.setTextColor(73, 80, 87);
        yPosition = renderTextWithLinks(doc, item.actions, 30 + actionsPadding, yPosition + actionsPadding - 2, pageWidth - 60 - actionsPadding * 2, 4);
        yPosition += 6;
      }
  ...
  // Don't add footer here - we'll add it in the loop below
  ...
  for (let i = 2; i <= totalPages; i++) {
    doc.setPage(i);
    addPageFooter(doc, i - 1, contentPages, pageWidth);
  }
